...
import com.sparta.spring_lv1.repository.UserRepository_beta;
...
@Service
public class UserService_beta {

    private final UserRepository_beta userRepository;

    private final PasswordEncoder passwordEncoder;

    private final JwtUtil jwtUtil;

    public UserService_beta(UserRepository_beta userRepository, PasswordEncoder passwordEncoder, JwtUtil jwtUtil) {
    ...
    public void signup(SignupRequestDto_beta requestDto) {  // 회원가입할 데이터를 requestDto로 받아온다
        ...
        // 회원 중복 확인
        Optional<User_bata> checkUsername = userRepository.findByUsername(username);  // 중복여부 확인하기
        ...
        Optional<User_bata> checkEmail = userRepository.findByEmail(email);
        ...
        // 사용자 등록
        User_bata user = new User_bata(username, password, email, role);  // 생성자 추가생성
        userRepository.save(user);
    }

    public void login(LoginRequestDto_beta requestDto, HttpServletResponse res) {
        String username = requestDto.getUsername();
        String password = requestDto.getPassword();

        // 사용자 확인
        User_bata user = userRepository.findByUsername(username).orElseThrow(  // 회원가입이랑 반대로 없을때가 오류임
